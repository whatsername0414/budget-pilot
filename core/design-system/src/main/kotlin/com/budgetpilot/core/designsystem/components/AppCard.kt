...
fun AppCard(
    ...
    val border = BorderStroke(HairlineBorderWidth, MaterialTheme.colorScheme.outlineVariant)
    if (onClick != null) {
        Card(onClick = onClick, modifier = modifier, shape = Shapes.large, border = border) {
            Column(modifier = Modifier.padding(contentPadding), content = content)
        }
    } else {
        Card(modifier = modifier, shape = Shapes.large, border = border) {
            Column(modifier = Modifier.padding(contentPadding), content = content)
        }
    }
}
